...
const recipes = [	{
	...
	},
	{
		...
		image: 'https://www.allrecipes.com/thmb/G1ETl0aSJafMaQAQIwV2AFR1Lzs=/1500x0/filters:no_upscale():max_bytes(150000):strip_icc()/12409-apple-crisp-II-MFS-033-beauty1-4x3-e7f6d6c193484e0daa43e571723ff16e.jpg',
		recipeIngredient: [
...
function search() {
  ...
  const filteredRecipes = userInput
    ? recipes.filter(recipe =>
        ...
        recipe.recipeIngredient.some(ing => ing.toLowerCase().includes(userInput))
      )
    : recipes;

  const sortedRecipes = [...filteredRecipes].sort(sortByRating);
...
function starsTemplate(rating) {
  let html = '<div class="card-stars" aria-label="' + rating + ' out of 5 stars">';
  for (let i = 1; i <= 5; i++) {
...
function recipeTemplate(recipe) {
  ...
  <div class="card-body">
    <span class="card-badge">${recipe.tags[0]}</span>
    <h2 class="card-title">${recipe.name}</h2>
    ${starsTemplate(recipe.rating)}
